...
describe("ErrorHandler", () => {
  ...
  it("should create an error on upload file", () => {
    const error = ErrorHandler.UploadFailed("Processing upload failed");
    expect(error.status).toBe(400);
    expect(error.message).toBe("Processing upload failed");
  });
});
